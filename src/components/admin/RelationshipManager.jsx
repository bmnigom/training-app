...
const RELATION_TYPES = ['Principal', 'Fuerza', 'Fisioterapia', 'Nutricion', 'Medico', 'Tecnico-tactico', 'Otro']
const ROLE_LABELS = { coach: 'Entrenador', nutritionist: 'Nutricionista', physio: 'Fisioterapeuta', doctor: 'Medico', admin: 'Admin' }
const ROLE_COLORS = { coach: 'bg-green-50 text-green-700', nutritionist: 'bg-orange-50 text-orange-700', physio: 'bg-teal-50 text-teal-700', doctor: 'bg-red-50 text-red-700', admin: 'bg-purple-50 text-purple-700' }
...
export default function RelationshipManager() {
    ...
    async function fetchAll() {
        ...
        setStaff(users.filter(u => {
            const roles = Array.isArray(u.roles) ? u.roles : u.role ? [u.role] : []
            return roles.some(r => ['coach', 'nutritionist', 'physio', 'doctor'].includes(r))
        }))
    ...
    return (
        <div className="space-y-6">
            <div>
                ...
                <p className="text-xs text-gray-400 mt-0.5">Asigna entrenadores, nutricionistas, fisioterapeutas y medicos a cada atleta</p>
            </div>
            ...
            <div className="bg-white rounded-2xl border border-gray-200 p-5 space-y-4">
                ...
                <div>
                    ...
                    <select value={form.athleteUid} onChange={e => setForm({ ...form, athleteUid: e.target.value })} className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
                ...
                <div>
                    ...
                    <select value={form.staffUid} onChange={e => setForm({ ...form, staffUid: e.target.value })} className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
                ...
                <button onClick={handleSave} disabled={saving || !form.athleteUid || !form.staffUid} className="w-full bg-blue-600 text-white rounded-xl py-2 text-sm font-medium hover:bg-blue-700 transition disabled:opacity-40">
                ...
                {athletes.map(athlete => {
                    ...
                    return (
                        <div key={athlete.uid} className="bg-white rounded-2xl border border-gray-200 p-4 space-y-3">
                            ...
                            {relations.length === 0 && <p className="text-xs text-gray-400">Sin equipo asignado</p>}
                            {relations.map(r => {
                        ...
                        <span className={"text-xs px-2 py-0.5 rounded-full font-medium " + (ROLE_COLORS[role] || 'bg-gray-100 text-gray-600')}>
                          {ROLE_LABELS[role] || role}
                                        ...
                                        <button onClick={() => handleDelete(r.id)} className="text-xs text-gray-400 hover:text-red-500 border border-gray-200 px-2 py-1 rounded-lg transition">
                                            Quitar
                                        </button>
                                    </div>
                                )
