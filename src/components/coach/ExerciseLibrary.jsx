    ...
    return (
        <div className="space-y-4 max-w-lg mx-auto">
          ...
          <div className="flex gap-3">
            ...
            <button onClick={handleSave} disabled={saving || !form.name.trim()} className={"flex-1 rounded-xl py-2 text-sm font-medium transition " + (saving ? 'bg-blue-400 text-white' : 'bg-blue-600 text-white hover:bg-blue-700 disabled:opacity-40')}>{saving ? 'Guardando...' : 'Guardar ejercicio'}</button>
          </div>
        </div>
    )
  }
  ...
  return (
      <div className="space-y-4">
          ...
          {filtered.map(ex => (
              <div key={ex.id} className="bg-white rounded-xl border border-gray-200 p-4 flex items-start justify-between gap-3">
                <div className="flex-1 min-w-0">
                  ...
                  {ex.videoUrl && (
                      <button
                          onClick={function(e) { e.stopPropagation(); setPreviewVideo(ex.videoUrl) }}
        ...
        {previewVideo && (
            ...
            >
              <div onClick={e => e.stopPropagation()} style={{ width: '90%', maxWidth: '640px', background: 'white', borderRadius: '16px', overflow: 'hidden' }}>
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '12px 16px', borderBottom: '1px solid #e5e7eb' }}>
                  <p style={{ fontSize: '14px', fontWeight: '500', color: '#1f2937', margin: 0 }}>Video de referencia</p>
                  <button onClick={() => setPreviewVideo(null)} style={{ fontSize: '20px', color: '#9ca3af', background: 'none', border: 'none', cursor: 'pointer', lineHeight: 1 }}>x</button>
                </div>
                {getEmbedUrl(previewVideo) ? (
                    ...
                    <div style={{ padding: '24px', textAlign: 'center', color: '#6b7280', fontSize: '14px' }}>
                      <p>No se puede mostrar este video.</p>
                      <a href={previewVideo} target="_blank" rel="noreferrer" style={{ color: '#3b82f6' }}>Abrir en YouTube</a>
                    </div>
                )}
              </div>
            </div>
